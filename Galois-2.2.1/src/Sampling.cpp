...
bool isInit;
bool isSampling;
__thread int papiEventSet = PAPI_NULL;
bool isEventSetInit=false;
...
static void begin(bool mainThread) {
  ...
  if (!isInit) {
    ...
    std::cout <<"PAPI VERSION: "<<PAPI_VER_CURRENT<<" rv: "<<rv<<std::endl;
    if (rv != PAPI_VER_CURRENT && rv > 0 ) { //&& rv > 0

      GALOIS_DIE("PAPI library version mismatch!");
    }
    if (rv < 0){ std::cout<<" ..."<<rv<<std::endl; GALOIS_DIE(PAPI_strerror(rv)); }
