...
static cll::opt<float> threshold("threshold", cll::desc("this is the update threshold that allows pushes (ratio)"));
    ...
    while (newDist < (unsigned int)(oldDist = ddata.dist)) {
      ...
      if (!UseCas || __sync_bool_compare_and_swap(&ddata.dist, oldDist, newDist | (oldDist & 0xffffffff00000000ul))) {
        ...
        /*if ((unsigned int) oldDist == (unsigned int)DIST_INFINITY ||
            (float) ((unsigned int) oldDist - (unsigned int) newDist) > threshold * newDist) {
        ...
        if ((unsigned int) oldDist == (unsigned int)DIST_INFINITY ||
          ...
          //if ((unsigned int) oldDist != (unsigned int)DIST_INFINITY)   {
          //  std::cout << (unsigned int) oldDist << "\t" <<(unsigned long) newDist << "\t" << (unsigned int) oldDist - (unsigned int) newDist << std::endl;
          
    //std::cout << "new dist is: " << newDist << std::endl;
          //}
            //  std::cout << &ddata << " being pushed with new Dist " << (unsigned int) newDist << std::endl;
          pusher.push(UpdateRequest(dst, newDist));
        }
