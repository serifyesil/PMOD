...
static cll::opt<std::string> outFile("outFile", cll::desc("This file is for error files"));
...
struct AsyncPri{
  ...
  struct PRPri {
    ...
    int operator()(const GNode& src, PRTy d) const {
      ...
      //std::cout<<"amp: "<<amp<<" pri: "<<(-1*d*amp)<<std::endl;
      return -1*d*amp;
    }
  ...
  struct ProcessWithBreaks {
    ...
    void operator()(const std::pair<GNode,PRTy>& srcn, Galois::UserContext<std::pair<GNode,PRTy>>& ctx) const { /*
      //ctx.t.stopwatch();
      GNode src = srcn.first;
      LNode& sdata = graph.getData(src);

      (*nOverall)+=1;
      if(sdata.residual < tolerance ||pri(src) < srcn.second){ // (pri(src) < srcn.second ||  (removed)
        *nEmpty += 1;
      ...
      for (auto jj = graph.edge_begin(src, lockflag), ej = graph.edge_end(src, lockflag); jj != ej; ++jj) {
        ...
        if(old + delta >= tolerance && (old <= tolerance || (pri(dst, old) > pri(dst,old+delta))))// || pri(dst, old) > pri(dst,old+delta))))
        {
          //std::cerr << "pri:"<<(pri(dst, old))<<" "<<(pri(dst, old+delta)) << "\n";
  ...
  struct Process {
    ...
    void operator()(const std::pair<GNode,int>& srcn, Galois::UserContext<std::pair<GNode,int>>& ctx) const {
      ...
      //heidars2 commented: PRTy oldResidual = sdata.residual.exchange(0.0);
      // heidars2
      ...
      for (auto jj = graph.edge_begin(src, lockflag), ej = graph.edge_end(src, lockflag); jj != ej; ++jj) {
        GNode dst = graph.getEdgeDst(jj);
        LNode& ddata = graph.getData(dst, lockflag);
        // heidars2 commented: PRTy old = atomicAdd(ddata.residual, delta);
        // heidars2 added:
        PRTy old = ddata.residual;
  ...
  void operator()(Graph& graph, PRTy tolerance, PRTy amp) {
    ...
    //typedef GlobPQ<std::pair<GNode, int>, LocalPQ<Comparer, std::pair<GNode, int>>> LPQ;
    ...
    auto fn2 = [&fppri] (const GNode& node) { return std::make_pair(node, fppri(node)); };
    ...
    //  Galois::for_each(boost::make_transform_iterator(graph.begin(), std::ref(fn2)),
    //                 boost::make_transform_iterator(graph.end(), std::ref(fn2)),
...
void runAsync() {
 ...
 // Open reference file and calculate errors here
  ...
  std::unordered_map<uint64_t, double> reference_rank;
  std::unordered_map<uint64_t, float> error_ratio;
  std::ifstream fd;
  fd.open(filename+".ref");
  if (!fd.is_open())
  {
    std::cout << "Reference file for PageRank not present. Can't open " << filename+".ref\n";
  }
  uint64_t label;
  double pagerank;
  while (fd >> label >> pagerank)
  {
     reference_rank.insert(std::pair<unsigned int, double> (label, pagerank)); 
  }
  fd.close();  
  ...
  for (auto N : graph)
  {
    ...
    float cur_error_ratio=std::fabs(reference_rank[N]-data.getPageRank())/reference_rank[N];
    int error_group_id = 19;
    if (cur_error_ratio/0.05 < 19)
      error_group_id = floor(cur_error_ratio/0.05);
    error_group[error_group_id]++;
    error_ratio.insert(std::pair<unsigned int, float> (N, cur_error_ratio)); 
  }
