...
static Galois::Statistic* RandWork;
...
struct AsyncAlgo {
  ...
  void relaxEdge(Graph& graph, Node& sdata, typename Graph::edge_iterator ii, Pusher& pusher) {
    //Galois::Runtime::TerminationDetection::terminateByForce();
    GNode dst = graph.getEdgeDst(ii);
    Dist d = graph.getEdgeData(ii);
    Node& ddata = graph.getData(dst, Galois::MethodFlag::NONE);
    Dist newDist = (unsigned int)sdata.dist + d;
    unsigned int new_frontier_id = (unsigned int)sdata.frontier_id + 1;
    
    if (ddata.frontier_id == 0 || (ddata.frontier_id != 0 && new_frontier_id < ddata.frontier_id))
      ddata.frontier_id =  new_frontier_id;

    Dist oldDist;
    
    // uncomment the folowing for selective push approximation II (do not drop
    ...
    while (newDist < (unsigned int)(oldDist = ddata.dist)) {
      if (!UseCas || __sync_bool_compare_and_swap(&ddata.dist, oldDist, newDist | (oldDist & 0xffffffff00000000ul))) {
        if (!UseCas)
          ddata.dist = newDist;
        if (ddata.frontier_id < maxFrontier || (unsigned int) oldDist == 4294967294) {
          /*typename Graph::iterator it = graph.begin();
          std::advance(it, startNode);
          Graph::GraphNode source = *it;
          it = graph.begin();
          std::advance(it, reportNode);
          Graph::GraphNode report = *it;  
...
struct AsyncAlgoPP {
  ...
  struct Initialize {
    ...
    void operator()(Graph::GraphNode n) {
      ...
      //heidars2: set the frontier id to be 0 initially
      g.getData(n, Galois::MethodFlag::NONE).frontier_id = 0;
    }
  };
...
void run(bool prealloc = true) {
  ...
  for (unsigned i = 0; i < graph.size(); i++)
  {
    ...
    std::advance(it2, i);
    current_node = *it2;
    //std::cout << i << "\t" << (unsigned int) graph.getData(current_node).frontier_id << std::endl;
    if ((unsigned int) graph.getData(current_node).frontier_id >= 11)
      error++;
  ...
  */ 
  ...
  for (unsigned i = 0; i < graph.size(); i++)
  {
    ...
    if ( dist == 4294967294 && reference_dist[i] != 4294967294)
      unvisited++;
    else
    {
...
int main(int argc, char **argv) {
  ...
  if (trackWork) {
    ...
    RandWork = new Galois::Statistic("RandWork");
