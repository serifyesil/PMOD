...
static cll::opt<bool> skipPrintDistances("skipPrintDistances", cll::desc("SkipPrintDistances"), cll::init(true)); 
...
static Galois::Statistic* nUpdateAttempts;
static Galois::Statistic* nUpdateSuccesses;
...
void run() {
  ...
  if (!skipPrintDistances) {
    ...
    for (unsigned i = 0; i < graph.size(); i++)
    {
      ...
      std::advance(it2, i);
      current_node = *it2;
      fd << i << "\t" << (unsigned int) graph.getData(current_node).dist << std::endl;
    } 
    fd.close();
  } 
}    
...
int  main(int argc, char **argv) {
  ...
  if  (trackWork) {
     ...
     nUpdateAttempts = new Galois::Statistic("nUpdateAttempts");
     nUpdateSuccesses = new Galois::Statistic("nUpdateSuccesses");
  }  
  ...
  if (trackWork) {
    ...
    delete nUpdateAttempts;
    delete nUpdateSuccesses;
  }
