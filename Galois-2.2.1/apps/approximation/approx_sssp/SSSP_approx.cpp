...
static Galois::Statistic* RecoveryWork;
static Galois::Statistic* nBad;
static Galois::Statistic* nEmpty;
static Galois::Statistic* nRecovery;
...
struct AsyncAlgo {
  ...
  void relaxEdge(Graph& graph, Node& sdata, typename Graph::edge_iterator ii, Pusher& pusher) {
    ...
    Dist oldDist;
    // uncomment the following for approx push optimization
    /*while (newDist < (unsigned int)(oldDist = ddata.dist)) {
      if (!UseCas || __sync_bool_compare_and_swap(&ddata.dist, oldDist, newDist | (oldDist & 0xffffffff00000000ul))) {
        if (!UseCas)
          ddata.dist = newDist;
        if (rand() % 100 <= pushParam)
          pusher.push(UpdateRequest(dst, newDist));
        break;
      }
    } */
    // uncomment the following for no CAS optimization
    if (newDist < (unsigned)(oldDist = ddata.dist)) {
    ddata.dist=newDist |  (oldDist & 0xffffffff00000000ul);
    pusher.push(UpdateRequest(dst, newDist)); 
    }
  ...
  void relaxNode(Graph& graph, UpdateRequest& req, Pusher& pusher) {
    ...
    else if (req.w < (unsigned int)*sdist) {  // someone has messed up (because they didn't use CAS), recover now. TODO: recovery is considered good work
      *sdist=req.w;
      *nRecovery += 1;
      *RecoveryWork += pusher.t.stopwatch();
      return;
    }
...
int main(int argc, char **argv) {
  ...
  if (trackWork) {
    ...
    RecoveryWork = new Galois::Statistic("RecoveryWork");
    nBad = new Galois::Statistic("nBad");
    nEmpty = new Galois::Statistic("nEmpty");
    nRecovery = new Galois::Statistic("nRecovery");
