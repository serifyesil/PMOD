...
template<class LocalHeap>
class ThreadAwareFreeListsHeap {
  PerThreadStorage<LocalHeap> heaps;

public:
  enum { AllocSize = LocalHeap::AllocSize };

  ThreadAwareFreeListsHeap() {}
  ~ThreadAwareFreeListsHeap() {
    clear();
  }

  inline void* allocate(size_t size, int list) {
    return heaps.getLocal()->allocate(size, list);
  }

  inline void deallocate(void* ptr, int list) {
    heaps.getLocal()->deallocate(ptr, list);
  }
...
template<class SourceHeap, int numFreeLists, bool doClear=true>
class FreeListsHeap : public SourceHeap {
  struct FreeNode {
    FreeNode* next;
  };
  FreeNode* head[numFreeLists];
...
//! Maintain a freelist
template<class SourceHeap, bool doClear=true>
...
typedef ThreadAwareFreeListsHeap<FreeListsHeap<SimpleBumpPtr<SystemBaseAlloc>, 24, false > > ListNodeHeap;
...
public:
  ...
  typedef ThreadAwarePrivateHeap<
    FreeListHeap<SimpleBumpPtr<SystemBaseAlloc>, false > > SizedAllocFastDstr;
